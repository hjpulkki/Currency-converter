...
def plusminus(sign):
 ...
 # Remove duplicates from the formula array. If two estimates give a rate within the err_tolerance, the more difficult one will be removed from the array.
def cleanDuplicates(estimates,  err_tolerance):
 ...
 while estimates:
  ...
  for est2 in estimates_new:
   if est1[0] < est2[0]*(1+err_tolerance) and est1[0] > est2[0]*(1-err_tolerance):
    if est1[1] >= est2[1]:
     flag = 0
    else:
...
def addEstimate(est1,estimates_new,estimates_final,diff_level,err_tolerance):
...
def generateEstimates(diff_level):
 ...
 while estimates_new:
  ...
  for operator in operators:
   ...
   [estimates_new, estimates_final] = addEstimate(new_estimate,estimates_new,estimates_final,diff_level,err_tolerance)
  ...
  if i%100 == 0:
   estimates_final = cleanDuplicates(estimates_final,err_tolerance)
